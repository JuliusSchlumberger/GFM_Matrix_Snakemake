...
which can only be discovered by actually invoking the model. Engine selected
by `simulation.engine` in the resolved config.yml, same as run_aqueduct.py -
...
def main() -> None:
    ...
    toml_path = os.path.join(inputs_dir, f"aqueduct_{scenario_name}.toml")
    ...
    if engine == "python":
        try:
            run_aqueduct_python(
                dem_path, mask_path, friction_path, boundaries_path, output_path,
