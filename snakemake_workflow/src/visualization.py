...
def load_statistics(statistics_path: str | Path) -> pd.DataFrame:
    """Load a wide per-geogunit exposure-statistics CSV, indexed by geogunit_id.

    Legacy format; unused by the current analysis pipeline, which reads
    compute_exposure_analysis.py's per-scenario File 1/2/3 CSVs directly
    (plain pd.read_csv — see analysis/plot_timeseries.py, plot_adaptation_bars.py,
    plot_burning_ember.py, plot_world_map.py for the exact column conventions).
    """
    df = pd.read_csv(statistics_path, index_col="geogunit_id")
    return df


def build_ssp_growth_for_entity(
    growth_df: "pd.DataFrame",
    iso_codes: list[str],
    ssps: list[str] | None = None,
) -> dict[str, "pd.Series"]:
    """Return {SSP: pd.Series(index=year, values=weighted_mean_growth_factor)}.

    For use as the `ssp_growth` argument to `plot_burning_ember` and
    `plot_timeseries` when plotting an aggregate (country or continent).

    The mean growth factor across `iso_codes` gives the "representative"
    trajectory for that entity.  If iso_codes contains a single country,
    this reduces to that country's own growth factor.

    Args:
        growth_df:  Per-(SSP, ISO, year) growth factor DataFrame from
                    `population_growth.load_ssp_growth_factors`.
        iso_codes:  ISO-3 codes to average over.
        ssps:       SSP labels to include (default: all available).

    Returns:
        Dict mapping SSP label → pd.Series(year → mean growth factor).
    """
    available_ssps = growth_df.index.get_level_values("scenario").unique().tolist()
    ssps = ssps or available_ssps
    result = {}
    for ssp in ssps:
        if ssp not in available_ssps:
            continue
        try:
            sub = growth_df.xs(ssp, level="scenario")
        except KeyError:
            continue
        matched = [iso for iso in iso_codes if iso in sub.index]
        if not matched:
            continue
        # Simple mean (could be population-weighted if pop data were passed)
        mean_series = sub.loc[matched].mean(axis=0)
        mean_series.index = mean_series.index.astype(int)
        result[ssp] = mean_series.sort_index()
    return result
...
def aggregate_to_country(df: pd.DataFrame) -> pd.DataFrame:
    """Sum all numeric columns by ISO country code."""
    numeric = df.select_dtypes(include="number").columns
    if "ISO" not in df.columns:
        raise ValueError("DataFrame must have an 'ISO' column for country aggregation.")
    return df.groupby("ISO")[numeric].sum()


def aggregate_to_continent(df: pd.DataFrame, region_col: str = "Region") -> pd.DataFrame:
    """Sum all numeric columns by continent / region label."""
    numeric = df.select_dtypes(include="number").columns
    if region_col not in df.columns:
        raise ValueError(f"DataFrame must have a '{region_col}' column.")
    return df.groupby(region_col)[numeric].sum()
...
def save_per_country_figures(
    ...
    slr_interp_mm: np.ndarray,
    output_dir: Path,
    chart_type: str = "burning_ember",
    slr_trajectories: pd.DataFrame | None = None,
    ssp_growth: dict[str, pd.Series] | None = None,
    cfg: dict | None = None,
    dpi: int = 200,
) -> None:
    """Generate one chart per ISO country, saving PNGs to output_dir.

    Args:
        df:              Full statistics DataFrame (with ISO column).
        eai_dict:        {measure: EAI DataFrame (geogunit rows, SLR cols)}.
        slr_scenarios:   SLR scenario column names (e.g. ["SLR_0", "SLR_200", …]).
        slr_mm_values:   mm values corresponding to slr_scenarios.
        growth_rates:    Growth rate grid for burning ember.
        slr_interp_mm:   Dense SLR interpolation grid.
        output_dir:      Destination directory.
        chart_type:      "burning_ember", "adaptation_bars", or "timeseries".
        slr_trajectories, ssp_growth: Required for "burning_ember" / "timeseries".
        cfg:             Optional viz config dict for colours/labels.
        dpi:             Figure resolution.
    """
    ...
    measure_colors = (cfg or {}).get("measure_colors", _DEFAULT_MEASURE_COLORS)

    iso_codes = df["ISO"].dropna().unique() if "ISO" in df.columns else []
    for iso in iso_codes:
        mask = df["ISO"] == iso
        gids = df.index[mask]

        if chart_type == "burning_ember":
            for measure, eai_df in eai_dict.items():
                eai_sub = eai_df.loc[eai_df.index.isin(gids), slr_scenarios]
                if eai_sub.empty or eai_sub.sum().sum() == 0:
                    continue
                matrix = build_growth_matrix(eai_sub, growth_rates, slr_mm_values,
                                             slr_interp_mm)
                vmax = float(matrix.max()) or 1.0
                fig = plot_burning_ember(
                    matrix, slr_interp_mm, growth_rates,
                    title=f"{iso} — {measure}",
                    slr_trajectories=slr_trajectories,
                    ssp_growth=ssp_growth,
                    cmap=cmap, vmax=vmax, dpi=dpi,
                    ssp_colors=ssp_colors,
                )
                fig.savefig(output_dir / f"burning_ember_{iso}_{measure}.png",
                            bbox_inches="tight")
                plt.close(fig)

        elif chart_type == "adaptation_bars":
            eai_sub_dict = {k: v.loc[v.index.isin(gids)] for k, v in eai_dict.items()}
            fig = plot_adaptation_bars(
                eai_sub_dict, slr_scenarios, slr_mm_values,
                entity_label=iso, measure_colors=measure_colors, dpi=dpi,
            )
            fig.savefig(output_dir / f"adaptation_bars_{iso}.png", bbox_inches="tight")
            plt.close(fig)


def save_per_continent_figures(
    df: pd.DataFrame,
    eai_dict: dict[str, pd.DataFrame],
    slr_scenarios: list[str],
    slr_mm_values: list[float],
    ...
    for reg in regions:
        gids = df.index[df[region_col] == reg]
        ...
        if chart_type == "burning_ember":
            for measure, eai_df in eai_dict.items():
                ...
                fig.savefig(output_dir / f"burning_ember_{safe}_{measure}.png",
                            bbox_inches="tight")
                plt.close(fig)
            ...
            fig = plot_adaptation_bars(
                eai_sub_dict, slr_scenarios, slr_mm_values,
                entity_label=reg, measure_colors=measure_colors, dpi=dpi,
            )
            safe = reg.replace(" ", "_").replace("/", "-")
            fig.savefig(output_dir / f"adaptation_bars_{safe}.png", bbox_inches="tight")
            plt.close(fig)
