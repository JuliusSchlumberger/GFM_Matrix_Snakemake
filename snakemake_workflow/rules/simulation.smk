...
rule run_aqueduct:
    ...
    The output filename includes `{waterlevel_name}` so it is clearly
    linked to the sea level rise scenario used to produce it.
    ...
    logged to `model_outputs/skipped_tiles/{tile_id}_{waterlevel_name}.txt`
    for later mapping.
    ...
    count, not the SLR scenario, so this is effectively a per-`tile_id`
    failure), the tile is marked in `model_outputs/oom_tiles/{tile_id}.txt`
    and this job's output falls back to the same all-`AQUEDUCT_NODATA`
    placeholder (logged to `skipped_tiles/` as above) instead of failing.
    Once marked, all other `waterlevel_name` scenarios for this `tile_id`
    skip Aqueduct entirely and do the same, instead of also running out of
    memory.
    """
    input:
    ...
    output:
        waterdepth=os.path.join(
            config["simulation"]["model_outputs"], "{tile_id}", "results", "waterdepth_{waterlevel_name}.tif"
        ),
    resources:
        aqueduct_runs=1,
    script:
