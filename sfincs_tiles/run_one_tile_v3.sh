...
# The trailing set label is optional (2026-09-24, user direction: no more
# Set A/B bookkeeping for new validation runs) - kept accepted here only for
# backward compatibility with the already-generated v2/v3 batch scripts,
# which do still pass one. A future run's batch scripts (generated by
# generate_v2_batch_jobs.py once its own default tile-ids file has no set
# column) will simply omit $2.
#
# --models (2026-09-24, user direction): comma-separated subset of
# bathtub,eikonal,sfincs to (re-)run this pass - e.g. "--models bathtub,sfincs"
# to skip eikonal entirely while tests/test_sweep_budget_calibration.py's own
# sweep-count calibration is still running, without losing bathtub/SFINCS
# progress. Default: all three. postprocess_tile_summary.py always runs
# regardless (it already degrades any missing model's own stats to null, see
# its own per-model `if path.exists()` checks - no flag needed there).
# --max-rounds: forwarded to run_eikonal_on_sfincs_subgrid.py's own
# --max-rounds (only meaningful when eikonal is in --models) - intended to
# eventually take the sweep-budget calibration study's own settled value.
set -uo pipefail
